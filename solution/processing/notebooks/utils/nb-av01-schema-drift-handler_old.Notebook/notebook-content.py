...
def _build_expected_schema(mapping):
    ...
    - nested=True for struct-felt – type-sjekk hoppar over desse
    """
    schema = {}
    for m in mapping:
        src = m.get("source", "")
        ...
        if top not in schema:
            ...
            schema[top]["nested"] = True   # merk struct viss sub-felt brukar dot
    return schema
    ...
    if not required and mapping:
        for m in mapping:
            ...
            if src and src != "_loading_ts" and m.get("include_in_load", True):
                required.add(src.split(".")[0])   # berre include-felt er required
