...
def detect_schema_drift(
    ...
    # Felt med include_in_load=False er kjente system-felt – aldri NEW_COLUMN
    # Brukar _top_level() for korrekt handtering av punktum i feltnamn.
    known = {
        _top_level(m["source"])[0]
        for m in mapping
        if not m.get("include_in_load", True)
    }
    unmatched_new = new_in_file - {n for _, n in rename_candidates} - known
    ...
    for col in expected:
        if col in actual_cols and col not in missing_in_file:
            if expected[col].get("nested"):
                continue   # struct-felt: typen gjeld sub-feltet, ikkje top-level
...
def _top_level(source: str) -> tuple:
    """
    Returnerer (top_level_name, is_nested) frå source-kolonnenamn.

    Ekte dot-notasjon (nesting):
      "snippet.title"            → ("snippet", True)
      "statistics.viewCount"     → ("statistics", True)
      "URL.Url"                  → ("URL", True)

    Punktum som del av feltnamnet (ikkje nesting):
      "Sum driftsinnt."          → ("Sum driftsinnt.", False)
      "Avskr. varige driftsmidl."→ ("Avskr. varige driftsmidl.", False)
      "Ord. res. f. skatt"       → ("Ord. res. f. skatt", False)

    Regel: ekte dot-notasjon har eit enkelt alfanumerisk ord FØR første punktum
    (ingen mellomrom, berre bokstavar/tal/underscore).
    """
    if "." not in source:
        return source, False
    first = source.split(".")[0]
    if " " not in first and first.replace("_", "").isalnum():
        return first, True   # ekte nesting
    return source, False     # punktum er del av feltnamnet


def _build_expected_schema(mapping):
    """
    Byggjer forventa top-level skjema frå column_mapping.
    - Felt med include_in_load=False vert ekskludert (kjente system-felt)
    - nested=True for ekte struct-felt (dot-notasjon)
    - Feltnamn med punktum som del av namnet vert behandla som flate felt
    """
    schema = {}
    for m in mapping:
        src = m.get("source", "")
        if src == "_loading_ts":
            continue
        if not m.get("include_in_load", True):
            continue   # kjent system-felt, ikkje del av forventa skjema
        top, is_nested = _top_level(src)
        if top not in schema:
            schema[top] = {"type": m.get("type", "string"), "nested": is_nested}
        elif is_nested:
            schema[top]["nested"] = True
    return schema
...
def _parse_required_fields(load_params, mapping=None):
    ...
    if not required and mapping:
        for m in mapping:
            src = m.get("source", "")
            if src and src != "_loading_ts" and m.get("include_in_load", True):
                top, _ = _top_level(src)
                required.add(top)   # brukar _top_level for punktum i feltnamn
...
def _check_type_compatibility(expected_type, actual_spark_type):
    """
    Ikkje flagg: numeric forventa, string i rådata
      → load_json_to_delta castar med .cast() og F.when(== "", None)
      → normal for Excel-eksport og nokre API-felt
    Flagg: boolean forventa, string i rådata
    Flagg: string forventa, numerisk i rådata (feltet har endra karakter)
    """
    numeric = {"int", "bigint", "double", "float", "long"}

    if expected_type in numeric and "string" in actual_spark_type:
        return None   # handtert av mapping-cast, ikkje reell drift

    if expected_type == "boolean" and "string" in actual_spark_type:
        return f"forventet boolean, fikk string"

    if expected_type == "string" and any(t in actual_spark_type for t in numeric):
        return f"forventet string, fikk {actual_spark_type}"
