...
%run nb-av01-schema-drift-handler
    ...
    for file in all_files:
        ...
        if not drift_report["compatible"]:
            ...
            try:
                ...
                suspended_path = f"{folder_path}suspended/{file.name}"
                notebookutils.fs.mkdirs(f"{folder_path}suspended/")
                notebookutils.fs.mv(file.path, suspended_path, overwrite=True)
                print(f"  -> Flytta til suspended/: {file.name}")
            except Exception as _se:
                print(f"  -> ADVARSEL: Kunne ikkje flytte til suspended/: {_se}")
            continue
