...
-- instructions tables.
--
-- Run order: After 06_migrate_add_handler_and_pipeline_columns.sql
-- Idempotent: truncates all tables before re-seeding
--
-- Data seeded:
--   Step 0:  TRUNCATE all seed tables
...
-- ============================================================================
-- STEP 0: TRUNCATE - clear all seed tables before re-seeding (idempotent)
-- Order matters: child tables first, then parent tables
-- ============================================================================
-- Child tables first (respect FK constraints)
DELETE FROM instructions.validations
DELETE FROM instructions.transformations
DELETE FROM instructions.loading
DELETE FROM instructions.ingestion
DELETE FROM metadata.column_mappings
DELETE FROM metadata.expectation_store
DELETE FROM metadata.transform_store
DELETE FROM metadata.loading_store
DELETE FROM metadata.source_store
DELETE FROM metadata.log_store
GO
...
VALUES
    (1, 'log_standard',
        'Standard logging - records start, end, row counts, and status',
        '{"params": ["pipeline_name", "notebook_name", "status", "rows_processed"]}'),
    (2, 'log_validation',
        'Validation logging - one row per expectation result with GX metadata',
...
-- ============================================================================
INSERT INTO metadata.source_store
    (source_id, source_name, source_type, auth_method, key_vault_url,
     secret_name, base_url, handler_function, description)
VALUES
...
-- ============================================================================
INSERT INTO instructions.ingestion
    (ingestion_id, source_id, endpoint_path, landing_path,
     request_params, is_active, log_function_id, pipeline_name, notebook_name)
VALUES
...
-- ============================================================================
INSERT INTO instructions.loading
    (loading_instr_id, loading_id, source_path, source_layer, target_table, target_layer,
     key_columns, load_params, merge_condition, merge_type, merge_columns,
     is_active, log_function_id, pipeline_name, notebook_name)
VALUES
...
-- ============================================================================
INSERT INTO instructions.transformations
    (transform_instr_id, source_table, source_layer, dest_table, dest_layer,
     transform_pipeline, transform_params, merge_condition, merge_type, merge_columns,
     is_active, log_function_id, pipeline_name, notebook_name)
VALUES
...
-- ============================================================================
INSERT INTO instructions.validations
    (validation_instr_id, target_table, target_layer, expectation_id,
     column_name, validation_params, severity, is_active,
     log_function_id, pipeline_name, notebook_name)
VALUES
...
-- YouTube Channels
INSERT INTO metadata.column_mappings (mapping_id, column_order, source_column, target_column, data_type, description)
VALUES
